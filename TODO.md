...
- [ ] **F7 (Major)** — the report route's swagger `400` still says `VALIDATION_ERROR` for a reversed range; F4 moved that case to `INVALID_DATE_RANGE`. The published contract names the wrong code
- [ ] F8 — `DocumentTotals` is defined but referenced by nothing; optionally have `Document` compose it via `allOf`
