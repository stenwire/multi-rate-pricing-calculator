...
| 2 | M4 was marked `[x]` without the `/verify` pass the skill requires; `VERIFICATION.md` still records only M0–M3. | Closing M4; M5 should not be stacked on an unaudited M4 | Open — M4 reverted to `[~]`. Clears once an M4-scoped verify run reports zero blockers. |
